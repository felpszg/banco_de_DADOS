...
SELECT venda_id, produto_id, observacao
FROM vendas_itens
WHERE
...
SELECT
    ...
    COALESCE(
        observacao,
        'Nenhuma observação'
    ) AS observacao --substitui dados que forem NULL por algum texto q vc quiser, isso é bao pra relatorios 
FROM vendas_itens
WHERE
    venda_id = 2001
...
SELECT venda_id, observacao
FROM vendas_itens
ORDER BY observacao ASC NULLS FIRST;
...
SELECT
    produto_id,
    SUM(valor_unitario) AS valor_total_produto,
